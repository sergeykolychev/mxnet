...
GetOptions(
    ...
    'load-prefix=i'  => \(my $load_prefix  = 'shakespeare'),
    'load-chkp=i'    => \(my $load_chkp    = 1       ),
...
method next()
{
    ...
    return undef if $self->counter == 100;
...
$fdata = pdl(map{ ord($_) } split(//, $fdata));
...
my $net1   = mx->sym->SoftmaxOutput(data => $pred, label => $label, name => 'softmax');
...
my ($net, $arg_params, $aux_params) = mx->rnn->load_rnn_checkpoint($stack, $load_prefix, $load_chkp);
...
my $score = $model->score($data_iter,
