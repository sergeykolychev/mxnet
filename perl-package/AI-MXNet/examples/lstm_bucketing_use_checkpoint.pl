...
=head1 SYNOPSIS
    ...
    --num-epochs     max num of epochs, default=0
...
my (undef, $arg_params, $aux_params) = mx->rnn->load_rnn_checkpoint($stack, 'lstm_buck', 25);
$model->fit(
    $data_train,
    eval_data           => $data_val,
...
my @out = $model->iter_predict($data_val, num_batch => 1);
use Data::Dumper;
my $pred = $out[0][0][0];
my $sentence_size = $pred->shape->[0]/$batch_size;
print "$batch_size $sentence_size\n";
my $vocab_size = scalar(keys %$vocabulary);
use Math::Random::Discrete;
for my $i (0..$batch_size-1)
{
    my @sentence;
    for my $j (0..$sentence_size-1)
    {
	my $ix = Math::Random::Discrete->new($pred->at($i*$sentence_size+$j)->aspdl->unpdl, [0..$vocab_size-1])->rand;
	#my $ix = $pred->at($i*$sentence_size+$j)->aspdl->maximum_ind;
	my $word = $reverse_vocab{ $ix };
	push @sentence, $word;
    }
    print join(' ', @sentence);
    print "\n-----\n";
...
print "Data\n";
my $data = $out[0][2]->data->[0]->aspdl;
for my $i (0..$batch_size-1)
{
    my @sentence;
    for my $j (0..$sentence_size-1)
    {
	my $word = $reverse_vocab{ $data->at($j,$i) };
	push @sentence, $word;
    }
    print join(' ', @sentence);
    print "\n-----\n";
...
print "label\n";
my $label = $out[0][2]->label->[0]->aspdl;
for my $i (0..$batch_size-1)
{
    my @sentence;
    for my $j (0..$sentence_size-1)
    {
	my $word = $reverse_vocab{ $label->at($j,$i) };
	push @sentence, $word;
    }
    print join(' ', @sentence);
    print "\n-----\n";
