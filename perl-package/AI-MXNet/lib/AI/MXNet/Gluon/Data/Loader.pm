...
=head1 NAME
    ...
    AI::MXNet::Gluon::Data::Loader - Dataset generator.
=cut
...
use strict;
use warnings;
package AI::MXNet::Gluon::Data::Loader;
...
sub BUILD
{
    ...
    if(not defined $self->batch_sampler)
    {
        ...
        if(not defined $self->sampler)
        {
            ...
            {
                $self->sampler(
                    AI::MXNet::Gluon::Data::RandomSampler->new(
            ...
            {
                $self->sampler(
                    AI::MXNet::Gluon::Data::SequentialSampler->new(
                        length => $self->dataset->len
                    )
                );
            }
        ...
        $self->batch_sampler(
            AI::MXNet::Gluon::Data::BatchSampler->new(
...
use overload
    '<>' => sub {
        ...
        return _batchify([map { $self->dataset->at($_) } @{ $batch }], $self->dataset->label->dtype);
    };
