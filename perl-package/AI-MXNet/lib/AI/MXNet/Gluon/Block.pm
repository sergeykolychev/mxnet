...
)
{
    if(not ref $fmt)
    {
        ...
        return ([@{$args}[0..$fmt-1]], [@{$args}[$fmt..@{$args}-1]]);
    }
    my @ret;
    for my $i (@{ $fmt })
    {
...
sub BUILD
{
    my $self = shift;
    $self->_prefix('');
    $self->_params(AI::MXNet::Gluon::ParameterDict->new(prefix => '', shared => $self->params));
    ...
    for my $i (@{ $out->list_auxilary_states })
    {
        if(not exists $input_names{$i})
        {
...
method forward($x, @args)
{
    ...
    assert(($in_fmt eq $self->_in_format), "Invalid input format");
    my $ret = $self->_cached_graph->[1]->copy;
    my %in;
    zip(sub {
        my ($k, $v) = @_;
        $in{$k->name} = $v;
